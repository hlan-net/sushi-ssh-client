...
- `CommandHistoryDatabaseHelper` — SQLite store following the `PhraseDatabaseHelper` pattern: `record()` inserts and prunes the host's oldest rows beyond 500, `search()` does case-insensitive text matching over command and output with an optional host filter, and `getRecentForHost()` feeds the AI context. The browser queries the store directly for the filter it is showing, so a write costs one insert and a prune — nothing reloads the whole list behind it.
- Recording points: `ConversationManager` records every command that reaches the shell (source `CONVERSATION` for AI-issued, `RAW` for Raw Terminal Mode), and `MainActivity.recordPlayInCommandHistory` records each Play's rendered command (source `PLAY`) once it has actually reached a shell — unless that command carries a secret (see below). Commands classified BLOCKED never execute, so they are never recorded.
...
- **Plays that substitute a secret parameter are not recorded.** `PlayRunner` reports `carriesSecretValues` when a parameter marked `secret` had a value to substitute, and `MainActivity.recordPlayInCommandHistory` skips the entry. The managed "change user password" play renders to `echo user:pass | sudo chpasswd`, so the rendered command holds the typed password in plaintext — and history entries are searched, copied to the clipboard and re-run. A redacted command would be worse than no entry: re-running it would send the mask to the shell as if it were the password.
...
- Output is stored condensed (first 5 non-blank lines, 500 characters including the ellipsis a truncated summary ends with) rather than in full.
