...
- [x] **`ConversationViewModel`** — owns the transcript, streaming output, CONFIRM state and the raw / auto-troubleshoot toggles as `StateFlow`; `ConversationManager` moves behind it. Adds the Compose BOM, since the next step uses it. The dialog keeps working beside it.
- [x] **`ConversationScreen`** — the first Compose screen, replacing the `AlertDialog`; `DialogGeminiControlsBinding` and the dialog code deleted in the same PR. `AiConversationTest` turned out to test `CommandSafety`/data classes/`TerminalSessionHolder`, not the dialog's UI — its `CommandSafety` coverage was already redundant with `CommandSafetyTest`, so it was retired; the rest moved to a JVM `TerminalSessionHolderTest` (no Android dependency, wrongly instrumented). `ConversationScreenTest` is the actual new Compose UI test, covering the screen itself. *(plan §5.8 · [Figma proposal](https://www.figma.com/design/heP71zbxhc6Mtgpghp0dDw/Sushi?node-id=103-2))*
...
- [ ] **Reboot Host does not reboot.** `ManagedPlays.kt` ships a built-in Play named *Reboot Host* whose script is `logout` and whose description says "Reboot placeholder". Make it `sudo reboot`. Plays run through `PlayRunner.execCommand` and do **not** pass `CommandSafety` — that classifier guards the AI conversation and raw mode, where `reboot` is BLOCKED, and that contract does not change. The Play therefore gets its own confirmation in the run dialog ("Reboot *ekho*? Open sessions will drop."), and the terminal treats the disconnect that follows as expected rather than as an error. *Interacts with auto-reconnect below: a reboot is a retryable disconnect, so reconnect's backoff should bring the session back once the host is up.*
...
- [ ] **Command history reaches cloud Gemini unredacted.** `ConversationContextBuilder` injects recent commands and their condensed output into the prompt. A `cat .env` in history sends its contents to Google with the next question. Regexes for known token shapes cannot make arbitrary output safe — `DATABASE_URL=…` or an AWS secret key has no reliable prefix — so the rule is **fail-closed on the source, best-effort on the rest**: (1) output of a command that reads a secret-bearing source is never sent to a cloud model — `env`, `printenv`, `export -p`, `set`, `cat`/`less`/`head`/`tail`/`grep`/`sed` on a path matching `*.env*`, `*.pem`, `*.key`, `id_*`, `*secret*`, `*credential*`, `*token*`, `/etc/shadow`, `~/.ssh/*`, `~/.aws/*`, `~/.config/gcloud/*`, plus `kubectl get secret*`, `aws configure list`, `docker inspect`; the prompt carries `[output withheld: may contain secrets]` in its place and the UI shows why; (2) everything else passes a `SecretRedactor` for known shapes (`AKIA…`, `ghp_…`, `sk-…`, `Bearer …`, `password=…`, PEM blocks) before it leaves the device; (3) the settings page says plainly that (2) is best effort. The on-device Nano path is unfiltered — keeping data on the device is what it is for. Unit-tested against a fixture of secret-bearing commands and outputs. *Does not change what is stored; storage is already excluded from backup.*
