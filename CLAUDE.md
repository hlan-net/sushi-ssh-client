...
**Migrated screens** — a `ViewModel` (`viewModelScope` + `StateFlow`) per screen owns the state, and a Compose screen renders it, mounted in the existing activity through `ComposeView` so navigation and intents keep working. The Compose BOM and the Compose compiler plugin are in the build since the `ConversationViewModel` PR (v0.9.0); `ConversationViewModel` in `net.hlan.sushi.conversation` is the reference for a state holder — `StateFlow<UiState>` plus intent functions, a `Channel` for one-off events, dependencies behind small interfaces so the class is JVM-testable. `ConversationScreen` (same package) is the reference for the screen itself: a pure function of `ConversationUiState` plus callbacks, so `ConversationScreenTest` feeds it state directly instead of driving the ViewModel through a live SSH session.

**Migration order:** the Gemini conversation first — it was an `AlertDialog` inside `MainActivity` and became the first Compose screen, `ConversationScreen` (`ROADMAP.md` §v0.9.0) — then `SettingsActivity` pages (self-contained, no SSH session), then `MainActivity`'s Plays tab, then its host list, then the Terminal tab last. `TerminalView` is a custom `AppCompatTextView` and stays as it is during this migration — a Compose screen that needs it wraps it in `AndroidView`; text selection, the IME connection and span rendering are the riskiest part of the app, and 38 releases of fixes live in it. The one thing that may replace it is the rewrite plan's terminal emulator (its Phases 2 and 5.7), and only if that plan is taken up; a migration PR never rewrites it.

**Each migration is its own PR that leaves the app releasable.** The screen's instrumented tests move with it, ported to a Compose UI test in place of Espresso view matchers — `createComposeRule()` feeding the screen state directly when it renders as a pure function of it (`ConversationScreenTest`), `createAndroidComposeRule<Activity>()` only when the screen genuinely needs its host — and its ViewModel gets JVM unit tests. That is the point of the exercise: logic that today can only be verified on a device becomes testable in `testDebugUnitTest`.
